<?php
  ...
  class DashboardTrailService extends TrailService
  {
     ...
     */
    public function getTrailForDashboard(Trail $trail, array $with = []): Trail
    {
      // TYLKO podstawowe relacje - bez automatycznego ładowania
      if (empty($with)) {
        // Domyślnie: nic nie ładuj lub tylko niezbędne
        return $trail;
      }

      // Załaduj tylko żądane relacje
      $allowedRelations = ['images', 'sections', 'points', 'riverTrack', 'points.pointType'];
      $relations = array_intersect($with, $allowedRelations);

      if (!empty($relations)) {
        $trail->load($relations);
      }
