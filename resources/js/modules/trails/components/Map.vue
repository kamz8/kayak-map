        ...
        >
            ...
            <l-marker-cluster :options="{ chunkedLoading: true }">
                <l-marker
                        ...
                        @popupopen="selectTrail(trail)"
                        @popupclose="clearActiveTrail"
                >
                    <l-popup>
                ...
                <l-marker
                        ...
                        :lat-lng="[activeTrail.end_lat, activeTrail.end_lng]"
                ></l-marker>
            </l-marker-cluster>
            <l-polyline
                    v-if="highlightedTrailCoords.length > 0 || activeTrailCoords.length > 0"
                    :lat-lngs="highlightedTrailCoords.length > 0 ? highlightedTrailCoords : activeTrailCoords"
                    :color="$vuetify.theme.current.colors['river-path']"
                    :weight="5"
                    :opacity="1"
                    :lineCap="'round'"
                    :lineJoin="'round'"
                    :class-name="'trail-path'"
                    :smoothFactor="0.5"
                    :stroke="true"
            />
        </l-map>
...
export default {
    ...
    methods: {
        ...
        highlightTrail(trail) {
            if (trail.river_track && !this.activeTrail) {
                this.highlightedTrailCoords = trail.river_track.track_points.map(point => [point[1], point[0]]);
            }
        },
        clearHighlightTrail() {
            if (!this.activeTrail) {
        ...
        selectTrail(trail) {
            ...
            this.activeTrailCoords = trail.river_track.track_points.map(point => [point[1], point[0]]);
        },
        clearActiveTrail() {
            this.activeTrail = null;
            this.activeTrailCoords = [];
            this.highlightedTrailCoords = [];
        },
        zoomIn() {
            this.zoom++;
        ...
        createEndMarkerIcon() {
            return new Icon({
                iconUrl: 'https://cdn.jsdelivr.net/npm/@mdi/svg/svg/flag-checkered.svg',
                iconSize: [30, 30],
                iconAnchor: [15, 30],
            });
        }
...
.trail-path {
    stroke: var(--v-theme-river-path-outline);
    stroke-width: 2px;
    stroke-opacity: 1;
    ...
    fill: none;
    filter:
            drop-shadow(-1px -1px 0px var(--v-theme-river-path-outline));

}

.trail-path.active {
    box-shadow: 0 0 10px 2px rgba(128, 111, 111, 0.5);
}
</style>
